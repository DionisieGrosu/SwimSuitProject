...
cartButton.addEventListener("click", function (e) {
  if (orderPopup.className.indexOf("_active") < 0) {
    ...
    fetch(link)
      ...
      .then((data) => {
        console.log(data);
        let orderPopupCartItemsWrap = document.querySelector(
          ".order-popup__cart-items"
        );
        ...
        for (let d = 0; d < data.productsCart.length; d++) {
          orderPopupCartItems += `<div class="order-popup__cart-item" data-id="${
            data.productsCart[d].id
          }" >
        <div class="order-popup__cart-item-image-wrap">
            <picture>
                <source srcset="${
                  data.productsCart[d].image[0]
                }" type="image/webp"><img src="${
            data.productsCart[d].image[0]
          }" alt=""></picture>
        </div>
        <div class="order-popup__cart-item-info">
            <span class="order-popup__cart-item-name">${
              data.productsCart[d].name
            }</span>
            <p class="order-popup__cart-item-size">Размез ${
              data.productsCart[d].size
            }</p>
            <p class="order-popup__cart-item-article">${
              data.productsCart[d].article
            }</p>
        </div>
        <div class="order-popup__cart-item-count-wrap">
            ...
            <input type="number" name="item-count" value="${
              data.productsCart[d].qty
            }">
            <button class="order-popup__cart-item-count-plus-btn"><i class="fas fa-plus"></i></button>
        </div>
        <span class="order-popup__cart-item-price">${
          Number(data.productsCart[d].price) * Number(data.productsCart[d].qty)
        }</span>
        <button class="order-popup__cart-item-delete-btn" data-url="${
          data.productsCart[d].deleteItemUrl
        }"><i class="fas fa-times"></i></button>
    </div>`;
        }
        let totalPrice = document.querySelector(
          ".order-popup__total-price span"
        );
        let totalItemsPrice = document.querySelector(
          ".order-popup__items-price span"
        );
        if (data.productsCart.length <= 1) {
          totalSum =
            Number(data.productsCart[0].price) *
            Number(data.productsCart[0].qty);
        } else {
          totalSum = data.productsCart.reduce((previousValue, currentValue) => {
            return (
              Number(previousValue.price) * Number(previousValue.qty) +
        ...
        let deleteItemEvent = document.querySelectorAll(
          ".order-popup__cart-item-delete-btn"
        );
        for (let e = 0; e < deleteItemEvent.length; e++) {
          deleteItemEvent[e].addEventListener("click", function (event) {
            ...
            if (event.target.className != "order-popup__cart-item-delete-btn") {
              deleteLink = event.target.closest(
                ".order-popup__cart-item-delete-btn"
              ).dataset.url;
            } else {
              deleteLink = event.target.dataset.url;
            }
            let productId = event.target.closest(".order-popup__cart-item")
              .dataset.id;
            fetch(deleteLink, {
              method: "POST",
              headers: {
              ...
              if (response.status == 200) {
                ...
                fetch(link)
                  ...
                  <span class="order-popup__cart-item-price">${
                    Number(data.productsCart[d].price) *
                    Number(data.productsCart[d].qty)
                  }</span>
                  <button class="order-popup__cart-item-delete-btn" data-url="${
                    data.productsCart[d].deleteItemUrl
                  }"><i class="fas fa-times"></i></button>
                    ...
                    for (let e = 0; e < deleteItemEvent.length; e++) {
                      deleteItemEvent[e].addEventListener(
                        "click",
                        function (event) {
                          let deleteLink;
                          if (
                          ...
                          let productId = event.target.closest(
                            ".order-popup__cart-item"
                          ).dataset.id;
                          fetch(deleteLink, {
                            method: "POST",
                            headers: {
                  ...
                  <button class="order-popup__cart-item-delete-btn" data-url="${
                    data.productsCart[d].deleteItemUrl
                                  ...
                                  orderPopupCartItemsWrap.innerHTML =
                                    orderPopupCartItems;

                                  let orderItemQtyMinusButton =
                                    document.querySelectorAll(
                                      ".order-popup__cart-item-count-minus-btn"
                                    );
                                  let orderItemQtyPlusButton =
                                    document.querySelectorAll(
                                      ".order-popup__cart-item-count-plus-btn"
                                    );

                                  for (
                                    let g = 0;
                                    g < orderItemQtyMinusButton.length;
                                    g++
                                  ) {
                                    orderItemQtyMinusButton[g].addEventListener(
                                      "click",
                                      function (event) {
                                        let inputField = event.target.closest(
                                          ".order-popup__cart-item-count-wrap"
                                        ).children[1];
                                          ...
                                          if (response.status == 200) {
                                            ...
                                            fetch(link)
                                              ...
                                              .then((data) => {
                                                ...
                                                if (
                                                  data.productsCart.length <= 1
                                                ) {
                                                  totalSum =
                                                    Number(
                                                      data.productsCart[0].price
                                                    ) *
                                                    Number(
                                                      data.productsCart[0].qty
                                                    );
                                                } else {
                                                  totalSum =
                                                    data.productsCart.reduce(
                                                      (
                                                        previousValue,
                                                        currentValue
                                                      ) => {
                                                ...
                                                totalPrice.innerHTML = totalSum;
                                                totalItemsPrice.innerHTML =
                                                  totalSum;
                                                let finalePriceValue =
                                                  Number(deliveryPriceValue) +
                                                  Number(totalSum);
                                                finalePrice.innerHTML =
                                                  finalePriceValue;
                                              });
                                          }
                                        });
                                  ...
                                  for (
                                    let g = 0;
                                    g < orderItemQtyPlusButton.length;
                                    g++
                                  ) {
                                    orderItemQtyPlusButton[g].addEventListener(
                                      "click",
                                      function (event) {
                                        let inputField = event.target.closest(
                                          ".order-popup__cart-item-count-wrap"
                                  ...
                                  if (data.productsCart.length <= 1) {
                                    ...
                                    totalSum = data.productsCart.reduce(
                                      (previousValue, currentValue) => {
                                        return (
                                          Number(previousValue.price) *
                                            Number(previousValue.qty) +
                    ...
                    let orderItemQtyMinusButton = document.querySelectorAll(
                      ".order-popup__cart-item-count-minus-btn"
                    );
                    let orderItemQtyPlusButton = document.querySelectorAll(
                      ".order-popup__cart-item-count-plus-btn"
                    );
                          ...
                          fetch(changeQtyLink, {
                            ...
                            if (response.status == 200) {
                              ...
                              fetch(link)
                                ...
                                .then((data) => {
                                  itemPrice = data.productsCart.filter(
                                    (item, index) => {
                                      return (
                                  ...
                                  itemQty = data.productsCart.filter(
                                    (item, index) => {
                                      return (
                                        Number(item.id) == Number(productId)
                                      );
                                  ...
                                  event.target.closest(
                                    ".order-popup__cart-item"
                                  ).children[3].innerHTML = itemPrice * itemQty;
                                  let totalPrice = document.querySelector(
                                    ".order-popup__total-price span"
                                  );
                                  if (data.productsCart.length <= 1) {
                    ...
                    totalItemsPrice.innerHTML = totalSum;
                    let finalePriceValue =
                      Number(deliveryPriceValue) + Number(totalSum);
                    finalePrice.innerHTML = finalePriceValue;
                  });
              }
            });
        ...
        let orderPopupDeliveryWrap = document.querySelector(
          ".order-popup__delivery-wrap"
        );

        let deliveryOptions = ``;
        for (
          let f = 0;
          f < data.deliveryOptions[0].deliveryOptions.length;
          f++
        ) {
        ...
        let deliveryOptionsEvent = document.querySelectorAll(
          ".order-popup__custom-radio input"
        );
        for (let t = 0; t < deliveryOptionsEvent.length; t++) {
          deliveryOptionsEvent[t].addEventListener("change", function (event) {
            let deliverySum = document.querySelector(
              ".order-popup__delivery-price span"
            );
            deliveryPriceValue = data.deliveryOptions[0].deliveryOptions.filter(
              (item, index) => {
                return Number(item.id) == Number(event.target.value);
              }
            )[0].deliveryPrice;
            ...
            finalePrice.innerHTML = finalePriceValue;
            console.log(
              data.deliveryOptions[0].deliveryOptions.filter((item, index) => {
                return Number(item.id) == Number(event.target.value);
              })
            );
          });
              ...
              fetch(changeQtyLink, {
                ...
                if (response.status == 200) {
                  ...
                  fetch(link)
                    ...
                    .then((data) => {
                      itemPrice = data.productsCart.filter((item, index) => {
                        return Number(item.id) == Number(productId);
                      })[0].price;
                      let totalItemsPrice = document.querySelector(
                        ".order-popup__items-price span"
                      );
                      itemQty = data.productsCart.filter((item, index) => {
                        return Number(item.id) == Number(productId);
                      })[0].qty;
                      event.target.closest(
                        ".order-popup__cart-item"
